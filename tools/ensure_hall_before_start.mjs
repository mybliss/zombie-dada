...
const screenshotPath = "/tmp/chrome_hall_check.png";
...
const hasStart = ocr.lines.some((line) => {
  const text = normalizeText(line.text);
  return text.includes(normalizeText("开始游戏")) || text.includes(normalizeText("开始"));
});
if (hasStart) {
...
const popupTitle = ocr.lines.find((line) => normalizeText(line.text).includes(normalizeText("组队邀请")));
if (popupTitle) {
  const bounds = getBounds();
  const imageWidth = bounds.width * 2;
  const imageHeight = bounds.height * 2;
  // Fixed close area for the invite popup in the current Chrome layout.
  ...
  const hallNow = retryOcr.lines.some((line) => {
    const text = normalizeText(line.text);
    return text.includes(normalizeText("开始游戏")) || text.includes(normalizeText("开始"));
  });
  if (hallNow) {
    console.log(JSON.stringify({ ok: true, state: "hall_after_close" }, null, 2));
    process.exit(0);
  }
}

console.error(JSON.stringify({ ok: false, reason: "hall_not_ready" }, null, 2));
process.exit(1);
